...
const test4 = () => {
  ...
  ${Object.keys(branches).reduce((result, branchName, index) => {
    return (result += `지점${index + 1} : ${branchName}(${
      branches[branchName]["addr"]
    })\n`);
  }, "")}`);
...
const printName = ({ name: { firstName, lastName } }) =>
